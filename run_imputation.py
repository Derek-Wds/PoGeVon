...
def get_model_classes(model_str):
    if model_str == 'pogevon':
        model, filler = models.PoGeVon, fillers.GraphVAEFiller
    else:
...
def get_dataset(args, dataset_name):
    ...
    elif dataset_name == 'pems_point':
        dataset = datasets.MissingValuesPems(args.subdataset_name, p_fault=0., p_noise=0.25)
    elif dataset_name == 'covid_point':
        dataset = datasets.MissingValuesCovid(p_fault=0., p_noise=0.25)
    else:
...
def parse_args():
    ...
    parser.add_argument("--model-name", type=str, default='pogevon')
    parser.add_argument("--dataset-name", type=str, default='air36')
    parser.add_argument("--subdataset-name", type=str, default='air36')
...
def run_experiment(args):
    ...
    # model's inputs
    if args.model_name == "pogevon":
        additional_model_hparams = dict(adj=adj, pos=positions, anchors=anchors, d_in=dm.d_in, n_nodes=dm.n_nodes)
    else:
