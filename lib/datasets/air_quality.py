...
class AirQuality(PandasDataset):
    ...
    def __init__(self, impute_nans=False, small=False, freq='60T', masked_sensors=None):
        ...
        self.test_months = [1,2,3,4]
        self.infer_eval_from = 'next'
        self.eval_mask = None
        df, dist, mask, adjs, positions, adj_label = self.load(impute_nans=impute_nans, small=small, masked_sensors=masked_sensors)
        self.dist = dist
        self.adjs = adjs
    ...
    def load_raw(self, small=False):
        ...
        adjs = np.load(os.path.join(datasets_path['air'], 'adjacency.npy'))
        positions = np.load(os.path.join(datasets_path['air'], 'position.npy'))
        adj_label = np.load(os.path.join(datasets_path['air'], 'adjacency_label.npy'))
        return df, stations, eval_mask, adjs, positions, adj_label

    def load(self, impute_nans=True, small=False, masked_sensors=None):
        # load readings and stations metadata
        df, stations, eval_mask, adjs, positions, adj_label = self.load_raw(small)
        # compute the masks
        ...
        return df, dist, mask, adjs, positions, adj_label
